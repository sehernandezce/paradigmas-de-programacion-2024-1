...
tryhaskell.preCommandHook = function(line,report){
    ...
    if (m = line.trim().match(/^step([0-9]+)/)) {
        ...
        report();
        return true;
    } else if (line.trim() == 'help' || line.trim() == 'ayuda' || line.trim() == '?') { //! Added "ayuda"
...
tryhaskell.makeController = function(){
    tryhaskell.controller = $('#console').console({
        ...
        animateScroll: true,
        promptHistory: true,
        welcomeMessage: 'Puedes probar instrucciones de Haskell aquí!',//'Puedes probar Haskell en tu navegador. Escribe "help" para empezar.',
